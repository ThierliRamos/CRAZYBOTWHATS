...
// Assumindo que a função está dentro de um comando
case "/pop":
  podeUsar = isDono ? true : vip ? true : false
  if(!podeUsar) return reply("🔐 Apenas pessoas autorizadas podem usar!")
await client.sendMessage(from, { text: "⚙️ Aguarde, estou gerando os resultados!" });

    const url = "https://www.worldometers.info/br/";
    ...
    try {
        ...
        const population = await page.$eval("span[rel='current_population']", el => el.innerText);
        const birthsThisYear = await page.$eval("span[rel='births_this_year']", el => el.innerText);
        const birthsToday = await page.$eval("span[rel='births_today']", el => el.innerText);
        const deathsThisYear = await page.$eval("span[rel='dth1s_this_year']", el => el.innerText);
        const deathsToday = await page.$eval("span[rel='dth1s_today']", el => el.innerText);
        const populationGrowthThisYear = await page.$eval("span[rel='absolute_growth_year']", el => el.innerText);
        const populationGrowthToday = await page.$eval("span[rel='absolute_growth']", el => el.innerText);


        // Formatar a resposta
        const responseMessage = `*Resultado agora:* ${dataAtual.getHours()}h e ${dataAtual.getMinutes()} minutos.\n  
🌍 *População mundial:* ${population}
👶 *Nascimentos este ano:* ${birthsThisYear}
👶 *Nascimentos hoje:* ${birthsToday}
💔 *Mortes este ano:* ${deathsThisYear}
💔 *Mortes hoje:* ${deathsToday}
📈 *Crescimento pop este ano:* ${populationGrowthThisYear}
📈 *Crescimento pop hoje:* ${populationGrowthToday}
        `;

        await client.sendMessage(from, { text: responseMessage });

    } catch (error) {
        console.error("Erro ao capturar dados: ", error);
        await client.sendMessage(from, { text: `Erro: ${error.message}` });
    } finally {
        if (browser) {
            await browser.close(); // Fecha o Puppeteer no final
        }
    }
    break;
